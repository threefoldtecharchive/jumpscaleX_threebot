...
export default ({
  ...
  actions: {
    getInstalledPackages: async (context) => {
      ...
      let response = await packageService.getInstalledPackages();
      for (let pkg of response.data.packages) {
        try {
          let routesResponse = await packageService.checkPathForFile(`${window.location.origin}/${pkg.name}/router.json`);
  ...
  installApp: (context, app) => {
    ...
    packageService.installApp(app).then(response => {
      ...
      setTimeout(function () { console.log("trying to get pkgs after timeout"), context.dispatch('getInstalledPackages') }, 2000)
