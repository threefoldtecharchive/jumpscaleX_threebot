...
from .rooter import abort, request, response, app, get_ws_url, package_route, PACKAGE_BASE_URL
...
def chat_handler(package, chat_name):
    ...
    if not chat_found:
        ...
        return env.get_template("error.html").render(package=package, error=error, email=session.get("email", ""))
    ...
    return env.get_template("index.html").render(
        ...
        noheader=query.get("noheader", True),
    )
