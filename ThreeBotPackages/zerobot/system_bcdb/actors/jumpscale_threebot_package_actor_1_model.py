...
class jumpscale_threebot_package_actor_1_model(j.baseclasses.threebot_actor):
    def _init(self, **kwargs):
        #get bcdb from package
        self.bcdb = j.data.bcdb.system
        self.model = self.bcdb.model_get(url="zerobot.system_bcdb.jumpscale.threebot.package.actor.1")
    ...
    def find(self, query=None,schema_out=None, user_session=None):
        ...
        ```
        ```out
        res = (LO) !jumpscale.threebot.package.actor.1
        ```
        """
        assert user_session.admin #for now only allow admin
