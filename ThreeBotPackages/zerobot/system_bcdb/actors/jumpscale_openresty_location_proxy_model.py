...
class jumpscale_openresty_location_proxy_model(j.baseclasses.threebot_actor):
    def _init(self, **kwargs):
        # get bcdb from package
        self.bcdb = j.data.bcdb.system
        self.model = self.bcdb.model_get(url="zerobot.system_bcdb.jumpscale.openresty.location_proxy")
    ...
    def new(self, schema_out=None, user_session=None, **kwargs):
        ...
        ```
        ```out
        res = (O) !jumpscale.openresty.location_proxy
        ```
        """
    ...
    def find(self, query=None, schema_out=None, user_session=None):
        ...
        ```
        ```out
        res = (LO) !jumpscale.openresty.location_proxy
        ```
        """
        assert user_session.admin  # for now only allow admin
