...
class SolutionExpose(j.servers.chatflow.get_class()):
    ...
    def domain_1(self):
        ...
        if "FreeTFT" == self.solution_currency:
            self.gateways = {
                ...
                if g.free_to_use and j.sal.zosv2.nodes_finder.filter_is_up(g)
            }
        else:
            self.gateways = {
                g.node_id: g for g in j.sal.zosv2._explorer.gateway.list() if j.sal.zosv2.nodes_finder.filter_is_up(g)
            }
    ...
    def confirmation(self):
        ...
        if not network:
            self.stop(
                f"Network {self.network_name} used by solution {self.user_form_data['Solution name']} has expired."
            )
