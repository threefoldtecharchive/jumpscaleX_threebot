...
class threefold_community_interest_1_model(j.baseclasses.threebot_actor):
    def _init(self, **kwargs):
        #get bcdb from package
        self.bcdb = self.package.bcdb
        self.model = self.bcdb.model_get(url="threefold.community_app.threefold.community.interest.1")
    ...
    def new(self,schema_out=None, user_session=None,**kwargs):
        ...
        ```
        ```out
        res = (O) !threefold.community_app.threefold.community.interest.1
        ```
        """
    ...
    def find(self, query=None,schema_out=None, user_session=None):
        ...
        ```
        ```out
        res = (LO) !threefold.community_app.threefold.community.interest.1
        ```
        """
        assert user_session.admin #for now only allow admin
