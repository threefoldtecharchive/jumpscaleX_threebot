...
class reservation_workload_1_model(j.baseclasses.threebot_actor):
    def _init(self, **kwargs):
        #get bcdb from package
        self.bcdb = self.package.bcdb
        self.model = self.bcdb.model_get(url="tfgrid.workloads.reservation.workload.1")
    ...
    def new(self,schema_out=None, user_session=None,**kwargs):
        ...
        workload_id = (s)
        user = (s)
        type = "zdb,container,volume,network" (e)
        content = (dict)
        created = (t)
        duration = (i)
        signature = (s)
    ...
    def find(self, query=None,schema_out=None, user_session=None):
        ...
        ```
        ```out
        res = (LO) !tfgrid.workloads.reservation.workload.1
        ```
        """
        assert user_session.admin #for now only allow admin
