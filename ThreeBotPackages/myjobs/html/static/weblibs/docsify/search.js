...
function saveData(config) {
  localStorage.setItem(resolveExpireKey(config.namespace), Date.now() + config.maxAge);
  localStorage.setItem(resolveIndexKey(config.namespace), JSON.stringify(INDEXS));
}
...
function init$1(config, vm) {
  ...
  var isExpired = localStorage.getItem(resolveExpireKey(config.namespace)) < Date.now();

  INDEXS = JSON.parse(localStorage.getItem(resolveIndexKey(config.namespace)));
